...
public class Listener extends ExtentReport implements ITestListener {
    ...
    public String getBase64() {
        ...
        */
        return ((TakesScreenshot) BrowserActions.driver).getScreenshotAs(OutputType.BASE64);
...
//        }
    ...
    public void onFinish(ITestContext context) {
        ...
        try {
            extent.flush();
            Desktop.getDesktop().browse(new File(System.getProperty("user.dir") + "/Extent Report/"+"Reoprt.html").toURI()); // it will open exetent report automatically once the execution is complete 
          // SendEmailWithAttachment.mailTheReport();  // to trigger email with report attached
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
